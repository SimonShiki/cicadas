...
export default function NowPlaying () {
    ...
    return (
        <>
            <div className='absolute bottom-0 left-0 flex w-full' onClick={() => {
                setFullscreen(true);
            }}>
                <Card className='!bg-white mx-auto mb-4 flex flex-col shadow-xl w-70vw lg:w-200 !p-0 overflow-hidden'>
                    ...
                    <div className='flex items-center m-4'>
                        <div className='flex flex-row gap-4 mr-auto'>
                            <img src={song.cover ?? defaultCover} alt={song.name} className='rounded-md w-10 h-10' />
                        ...
                        <div className='flex flex-row gap-2 color-text-pri items-center'>
                            <span className='i-fluent:previous-24-filled w-4 h-4' onClick={player.previous} />
                            ...
                            <span className='i-fluent:next-24-filled w-4 h-4' onClick={player.next} />
                        </div>
            ...
            {fullscreen && (
                <div className='absolute top-0 left-0 w-full h-full h-full' style={{ backgroundImage: `url("${song.cover}")`}}>
                    <div className='w-full h-full backdrop-filter backdrop-blur-256 bg-black bg-op-40 flex flex-col items-center justify-center'>
                        <img src={song.cover} className='shadow-md border-outline-pri rounded-md w-80 h-80' />
                        <div className='absolute bottom-0 w-full h-20 mt-auto py-4 bg-black bg-op-20 border-t-(1 solid text-sec) border-op-40'>
                            ...
                            <div className='flex flex-row px-6 py-2'>
                                <div className='flex flex-col grow-0.5 w-0'>
                                    <span className='color-white'>{song.name}</span>
                                    <span className='color-outline-sec font-size-sm'>{song.album}</span>
                                </div>
                                <div className='flex flex-row gap-4 color-white items-center'>
                                    <span className='i-fluent:previous-24-filled w-6 h-6' onClick={player.previous} />
                                    <span onClick={handlePlayPause} className={`${playing ? 'i-fluent:pause-24-filled' : 'i-fluent:play-24-filled'} w-8 h-8`} />
                                    <span className='i-fluent:next-24-filled w-6 h-6' onClick={player.next} />
                                </div>
                                <div className='absolute flex gap-2 pt-3 right-4 items-center color-white'>
