...
export default forwardRef<HTMLButtonElement, ButtonProps>(function Button ({
    ...
    const variantClasses = {
        primary: 'bg-fg-pri color-white hover:bg-[#a36fae] hover:!border-[#a36fae] active:color-op-60 active:bg-[#a36fae] active:!border-[#a36fae] !border-fg-pri disabled:bg-[#99ceff] dark:disabled:bg-[#0a4380] disabled:!border-opacity-60 disabled:hover:bg-[#99ceff] dark:disabled:hover:!bg-[#0a4380] disabled:active:bg-[#99ceff] dark:disabled:active:bg-[#0a4380]',
        secondary: 'bg-white dark:bg-bg-dark-pri color-text-pri dark:color-text-dark-pri hover:bg-bg-pri dark:hover:bg-bg-dark-sec border-outline-pri dark:border-outline-dark-pri active:bg-bg-sec dark:active:bg-bg-dark-sec disabled:hover:bg-white disabled:dark:hover:bg-black disabled:active:bg-white dark:disabled:active:bg-black',
    ...
    return (
        <button
            ...
            type={type}
            ref={ref}
            className={`font-inherit border-b-1.5 outline-none hover:cursor-pointer disabled:color-op-60 disabled:hover:cursor-not-allowed font-(400 size-sm) text-nowrap border-(1 solid) ${rounded ? 'rounded-full' : 'rounded-1.5'} transition-all ${variantClasses[variant]} ${iconOnly ? '' : gapMap[size]} ${className ?? ''}`}
        >
            {children}
        </button>
    );
